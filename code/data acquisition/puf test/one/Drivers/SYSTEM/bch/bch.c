                ...
                uint8_t *ecc)
{
    ...
    while (mlen--) {
        ...
        for (i = 0; i < l; i++) r[i] = r[i + 1] ^ p0[i] ^ p1[i] ^ p2[i] ^ p3[i];
                              ...
                              unsigned int *syn)
{
    ...
    for (j = 0; j < t; j++) syn[2 * j + 1] = gf_sqr(bch, syn[j]);
}
...
#endif
    if ((m < min_m) || (m > BCH_MAX_M)) goto fail;

    if (t > BCH_MAX_T) goto fail;
...
void bch_free(struct bch_control *bch)
{
    ...
    if (bch) {
        ...
        for (i = 0; i < ARRAY_SIZE(bch->poly_2t); i++) free(bch->poly_2t[i]);
